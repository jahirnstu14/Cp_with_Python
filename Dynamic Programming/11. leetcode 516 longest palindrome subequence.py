# nice solution : https://takeuforward.org/data-structure/longest-palindromic-subsequence-dp-28/

class Solution:
    def lcs(self, s1, s2):
        n = len(s1)
        m = len(s2)

        # Initialize a 2D array to store the length of the LCS
        dp = [[0] * (m + 1) for _ in range(n + 1)]

        # Fill in the dp array using dynamic programming
        for ind1 in range(1, n + 1):
            for ind2 in range(1, m + 1):
                if s1[ind1 - 1] == s2[ind2 - 1]:
                    dp[ind1][ind2] = 1 + dp[ind1 - 1][ind2 - 1]
                else:
                    dp[ind1][ind2] = max(dp[ind1 - 1][ind2], dp[ind1][ind2 - 1])

        # The final value in dp will be the length of the LCS
        return dp[n][m]

    def longestPalindromeSubseq(self, s):
        # Reverse the input string
        t = s[::-1]

        # Find the longest common subsequence between s and its reverse
        return self.lcs(s, t)

# For local testing:
def main():
    s = "bbabcbcab"
    
    solution = Solution()
    result = solution.longestPalindromeSubseq(s)
    print("The Length of Longest Palindromic Subsequence is", result)

if __name__ == "__main__":
    main()
